...
@Serializable
data class ApiGeometryResponse(
    val type: String,
    val coordinates: List<Double>
) {
    fun toModelGeometry() = when (type) {
        "Point" -> com.nars.maplibre.data.model.PointGeometry(type, coordinates)
        "LineString" -> com.nars.maplibre.data.model.LineStringGeometry(type, coordinates)
        "Polygon" -> com.nars.maplibre.data.model.PolygonGeometry(type, coordinates)
        "Circle" -> com.nars.maplibre.data.model.CircleGeometry(type, coordinates)
        else -> com.nars.maplibre.data.model.PointGeometry(type, coordinates)
    }
}
...
data class ApiPropertiesResponse(
    ...
    fun toFeatureProperties(featureType: NarsFeatureType): FeatureProperties {
        return when (featureType) {
            NarsFeatureType.ROAD -> FeatureProperties.RoadProperties(
                name = name, phase = phase ?: "roads", color = color ?: "#3498db",
                roadTypeKey = roadTypeKey, roadTraffic = roadTraffic,
                tradActivity = tradActivity, numLanes = numLanes,
                hasMedian = hasMedian, hasVegetation = hasVegetation,
                isDeadEnd = isDeadEnd, hasSidewalk = hasSidewalk
            )
            NarsFeatureType.HOUSE_ENTRANCE -> FeatureProperties.HouseEntranceProperties(
                name = name, phase = phase ?: "houseEntrances", color = color ?: "#27ae60",
                entranceTypeKey = entranceTypeKey, roadDbId = roadDbId,
                side = side, hasEntrance = hasEntrance,
            ...
            NarsFeatureType.NAMING_PANEL -> FeatureProperties.NamingPanelProperties(
                name = name, phase = phase ?: "namingPanels", color = color ?: "#9b59b6",
...
@Serializable
data class SaveFeatureResponse(
    val id: String? = null,
    val success: Boolean = true
)

@Serializable
data class CreateEntranceResponse(
    val id: String? = null,
    val success: Boolean = true
)

fun NarsFeature.toSaveFeatureRequest(): SaveFeatureRequest {
    val props = when (val p = properties) {
        is FeatureProperties.RoadProperties -> ApiPropertiesRequest(
            name = p.name, phase = p.phase, color = p.color,
            roadTypeKey = p.roadTypeKey, roadTraffic = p.roadTraffic,
            tradActivity = p.tradActivity, numLanes = p.numLanes,
            hasMedian = p.hasMedian, hasVegetation = p.hasVegetation,
            isDeadEnd = p.isDeadEnd, hasSidewalk = p.hasSidewalk
        )
        is FeatureProperties.HouseEntranceProperties -> ApiPropertiesRequest(
            name = p.name, phase = p.phase, color = p.color,
            entranceTypeKey = p.entranceTypeKey, roadDbId = p.roadDbId,
            side = p.side, hasEntrance = p.hasEntrance,
        ...
        is FeatureProperties.NamingPanelProperties -> ApiPropertiesRequest(
            name = p.name, phase = p.phase, color = p.color,
